...
| **F14** | **Dashboard de consumo de API** | ✅ Nueva vista `/uso`: cada llamada LLM/embedding queda logueada en `data/uso.jsonl` con tokens + costo USD (OpenRouter devuelve `cost` nativo). Dashboard muestra KPIs (mes actual + proyección), breakdown por feature/modelo/proyecto, gráfico de costo por día (30d), top archivos consumidores, y log de últimos eventos. Auto-refresh cada 15s. |
